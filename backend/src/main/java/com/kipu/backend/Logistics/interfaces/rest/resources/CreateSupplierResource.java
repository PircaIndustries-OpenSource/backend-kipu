...
@Schema(description = "Request resource used to create a supplier")
public record CreateSupplierResource(
        @Schema(description = "RUC (11 digits)", example = "20123456789", requiredMode = Schema.RequiredMode.REQUIRED)
        @NotBlank(message = "supplier.error.ruc.notBlank")
        @Pattern(regexp = "^\\d{11}$", message = "supplier.error.ruc.pattern")
        String ruc,
